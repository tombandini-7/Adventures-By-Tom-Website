...
import { disneylandEvents } from '../../data/disneyland';
      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        >
          ...
          <p className="text-gray-600 text-lg leading-relaxed">
            Throughout the year, Disneyland Resort hosts incredible seasonal events and festivals. Plan your visit around these magical celebrations!
          </p>
        </div>

        {/* Events Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {disneylandEvents.map((event, index) => (
...
interface EventCardProps {
  event: typeof disneylandEvents[0];
  delay: number;
}
...
const EventCard = ({ event, delay }: EventCardProps) => {
  ...
  const parkColors: Record<string, string> = {
    'Disneyland Park': 'bg-magenta',
    'Disney California Adventure': 'bg-ocean',
    'Both Parks': 'bg-aqua-dark',
  };

  const parkLabels: Record<string, string> = {
    'Disneyland Park': 'Disneyland',
    'Disney California Adventure': 'DCA',
    'Both Parks': 'Both Parks',
  };
    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="flex flex-wrap gap-2">
          {event.highlights.slice(0, 4).map((highlight) => (
            <span
              key={highlight}

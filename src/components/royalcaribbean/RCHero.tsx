...
const RCHero = () => {
  ...
  const [currentImageIndex, setCurrentImageIndex] = useState(() =>
    Math.floor(Math.random() * ASSETS.royalCaribbean.heroes.length)
  );
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentImageIndex((prev) =>
        (prev + 1) % ASSETS.royalCaribbean.heroes.length
      );
    }, 8000);
  ...
  return (
    <section id="rc-hero" className="relative h-[75vh] min-h-[450px] md:h-screen md:min-h-[600px] flex items-center justify-center overflow-hidden">
      {/* Background Images with Crossfade */}
      {ASSETS.royalCaribbean.heroes.map((image, index) => (
        <div
          key={image}
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
        ...
        <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl xl:text-7xl text-white mb-6 animate-fade-in">
          Where <span className="text-yellow italic">Adventure</span> Meets the Sea
        </h1>
        <p className="text-white/90 text-lg md:text-xl lg:text-2xl mb-8 max-w-2xl mx-auto leading-relaxed animate-fade-in-up">
          Experience the world's most innovative cruise ships, private island paradises, and unforgettable destinations across the globe.
        </p>
